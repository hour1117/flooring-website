...
export default function Footer({ locale, settings, categories }: Props) {
  const t = footerLabels[locale] || footerLabels.en;
    ...
    <footer className="bg-black text-white">
      <div className="container-custom py-20 md:py-28">
        <div className="grid gap-16 md:grid-cols-4">
          ...
          <div>
            <h4 className="text-xs font-medium uppercase tracking-[0.2em] text-neutral-500 mb-6">{t.products}</h4>
            <ul className="space-y-3">
              {categories.slice(0, 5).map((cat) => (
                <li key={cat.slug}>
                  <Link href={`/categories/${cat.slug}`} className="text-sm font-light text-neutral-300 hover:text-white transition-colors">
          ...
          <div>
            <h4 className="text-xs font-medium uppercase tracking-[0.2em] text-neutral-500 mb-6">{t.contact}</h4>
            <ul className="space-y-3">
            ...
            <div className="mt-6 flex gap-3">
              {settings.social.facebook && (
                <a href={settings.social.facebook} target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-white transition-colors" aria-label="Facebook">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
              )}
              {settings.social.instagram && (
                <a href={settings.social.instagram} target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-white transition-colors" aria-label="Instagram">
              ...
              {settings.social.linkedin && (
                <a href={settings.social.linkedin} target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-white transition-colors" aria-label="LinkedIn">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
              )}
              {settings.social.youtube && (
                <a href={settings.social.youtube} target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-white transition-colors" aria-label="YouTube">
      ...
      <div className="border-t border-white/10 py-6">
        <div className="container-custom text-center text-xs font-light text-neutral-500">
          &copy; {new Date().getFullYear()} {localizedValue(settings.company_name, locale)}. All rights reserved.
        </div>
      </div>
    </footer>
  );
